...
import { deleteProduct, fetchProduct, type Product, type ProductResponse } from '../api/products';
...
const ProductDetailPage: Component = () => {
    ...
    const parentProductId = createMemo(() => currentProduct()?.parent_product_id ?? null);
    ...
    const childrenAreLoading = createMemo(() => childQueries().some((q) => q.isPending));
    const childrenError = createMemo(() => childQueries().find((q) => q.isError)?.error ?? null);

    const hasChildren = createMemo(() => (currentProduct()?.child_product_ids.length ?? 0) > 0);
            ...
            <Paper class="hero-panel" elevation={0}>
                <Stack spacing={2}>
                    ...
                    <Show
                        ...
                        fallback={<Alert severity="error">Product id is missing in route.</Alert>}
                    >
                        <Show
                        ...
                        >
                            ...
                            >
                                <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2 }}>
                                    <Stack spacing={1.5}>
                                        <Typography variant="overline" sx={{ color: '#446a8f', letterSpacing: '0.12em' }}>
                                            Product detail
                                        </Typography>
                    ...
                    <Stack spacing={2}>
                        ...
                        >
                            ...
                            <Typography color="text.secondary" variant="body2">
                                ...
                                {(currentProduct()?.child_product_ids.length ?? 0) === 1 ? '' : 's'}
                            </Typography>
                        </Stack>
                                        ...
                                        <For each={children()}>
                                            {(child) => (
                                                <Box class="product-detail-child-row">
                                                    ...
                                                    >
                                                        <Stack spacing={0.25}>
                                                            <A class="product-inline-link product-inline-link-strong" href={`/products/${child.id}`}>
                                                                {child.name}
                                                            </A>
                                                            <Typography color="text.secondary" variant="body2">
                                                                {child.id}
